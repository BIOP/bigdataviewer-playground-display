...
public class BdvSupplierExample implements IBdvSupplier {
    ...
    public BdvHandle get() {
        ...
        bdv.getViewerPanel().state().changeListeners().add(new ExampleListener(bdv));
        ...
        // Collapse group card
        bdv.getCardPanel().setCardExpanded(BdvDefaultCards.DEFAULT_SOURCEGROUPS_CARD, false);
    ...
    public String toString() {
        return this.getClass().getSimpleName() + "| tp = " + numberOfTimepoints + " interpolate = " + interpolate;
    }
    ...
    public static class ExampleListener implements ViewerStateChangeListener {
        ...
        public void viewerStateChanged(ViewerStateChange change) {
            IJ.log(BdvHandleHelper.getWindowTitle(bdvh) + " state has changed : "+change);
            if (change.equals(ViewerStateChange.NUM_SOURCES_CHANGED)) {
                ...
                if (nSources>1) IJ.log("There are now "+bdvh.getViewerPanel().state().getSources().size()+" sources in this window");
            }
        }
    }
