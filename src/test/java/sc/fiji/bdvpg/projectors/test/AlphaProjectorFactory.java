...
public class AlphaProjectorFactory implements
	AccumulateProjectorFactory<ARGBType>
{
	...
	{
		return new AccumulateProjectorARGBGeneric(meta, sourceProjectors, sources,
			sourceScreenImages, targetScreenImage, numThreads, executorService);
	}
		...
		AccumulateProjector<ARGBType, ARGBType>
	{
		...
		final boolean[] is_alpha; // flags if the source in an alpha source ( not
															// displayed )
		final boolean[] has_alpha; // flag if the source has an alpha channel,
																// present in the list of sources
		final int[] sources_alpha_index; // index of the alpha channel, if any (
																			// has_alpha is false otherwise )

		public AccumulateProjectorARGBGeneric(SourcesMetadata meta,
			final List<VolatileProjector> sourceProjectors,
			final List<SourceAndConverter<?>> sources,
			final List<? extends RandomAccessible<? extends ARGBType>> sourceScreenImages,
			...
			final ExecutorService executorService)
		{
			...
			// Let's sort which sources are alpha, which are not, and which contain
			// alpha channels, actually present in the projector
			for (int index_source = 0; index_source < sources
				.size(); index_source++)
			{
				...
				if (meta.isAlphaSource(source)) {
					...
					if (meta.hasAlphaSource(source)) {
						...
						index_alpha_source = sources.indexOf(meta.getAlphaSource(source)); // returns
																																								// -1
																																								// if
																																								// the
																																								// source
			...
			for (int iSource = 0; iSource < accesses.length; iSource++) {
				if (!is_alpha[iSource]) {
					...
					if (has_alpha[iSource]) {
						...
						final Cursor<? extends ARGBType> access_alpha =
							accesses[sources_alpha_index[iSource]];
