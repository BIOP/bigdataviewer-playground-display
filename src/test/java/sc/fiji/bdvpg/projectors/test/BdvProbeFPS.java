...
public class BdvProbeFPS {
	...
	public static void acquireFrames(final int width, final int height,
		final int nFrames, ViewerPanel viewer)
	{
		final ViewerState renderState = new BasicViewerState(viewer.state()
			.snapshot());
		...
		final MultiResolutionRenderer renderer = new MultiResolutionRenderer(target,
			() -> {}, new double[] { 1 }, 0, 1, null, false, viewer.getOptionValues()
				.getAccumulateProjectorFactory(), new CacheControl.Dummy());
